...
float WaypointFollower::delivery_calcSteeringAngle()
{
	getClosestWaypoint(cur_pose_);

	lookahead_dist_ = cur_speed_ * ld_ratio + 1.0 + 3.0; // km/h //AD +4.0을 추가로해줌 spd*0.4+4=9
	...
	if (final_ld > maximum_ld)
		final_ld = maximum_ld; //AD
	if (final_ld < minimum_ld)
		final_ld = minimum_ld+3.0; //AD

	float cur_steer = atan2f(2.0f * wheel_base_ * sinf(alpha) / (final_ld), 1.0f);
	cur_steer = cur_steer * 180.0f / M_PI;


	// if(cur_steer>=19.99f) cur_steer=19.99f; //AD
	// if(cur_steer<=-29.99f) cur_steer=-29.99f;
	
	// if(lane1_stop_cnt==1 || lane2_stop_cnt==1)
	// {
	// 	if(cur_steer<=-19.99f) cur_steer=-19.99f;
	// 	if(cur_steer>=29.99f) cur_steer=29.99f;
	// }
	...
	if (cur_steer >= 29.99f)
	cur_steer = 29.99f;
	else if(cur_steer<=-29.99f)
	cur_steer=-29.99f;
	// if (cur_steer <= -14.99f)
	// 	cur_steer = -14.99f;
	cout<<"final_ld: "<<final_ld<<endl;
	cout<<"my_steering_angle: "<<cur_steer<<endl;
	return cur_steer;
}
